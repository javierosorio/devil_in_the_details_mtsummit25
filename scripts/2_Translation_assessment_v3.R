...
# Download the data 
download.file(
  url = "https://github.com/javierosorio/devil_in_the_details_mtsummit25/raw/main/data/text/data_master_text_clean_2.xlsx",
  destfile = "data_master_text_clean_2.xlsx",
  mode = "wb"
)

# Get the data
data.na <- read_excel("data_master_text_clean_2.xlsx")  
...
# Download the data 
download.file(
  url = "https://github.com/javierosorio/devil_in_the_details_mtsummit25/raw/main/data/quality/master_quality_data_ID_COMET_Updated.xlsx",
  destfile = "master_quality_data_ID_COMET_Updated.xlsx",
  mode = "wb"
)

# Get the data
data.qual <- read_excel("master_quality_data_ID_COMET_Updated.xlsx")
...
data.qual.mean <- data.qual %>%
  ...
  summarise(SacreBLEU = mean(SacreBLEU),
            METEOR = mean(METEOR),
...
data.qual.mean <- data.qual.mean %>%
  pivot_longer(cols=c("SacreBLEU","METEOR","COMET","BERTScore"),
               names_to="metric",
               values_to="values")
...
data.qual.mean <- data.qual.mean %>%
  mutate(metric=case_when(
    metric=="SacreBLEU" ~ "(a) SacreBLEU",
    metric=="METEOR" ~  "(b) METEOR",
    metric=="COMET" ~ "(c) COMET",
    metric=="BERTScore" ~ "(d) BERTScore"  
  ))
...
ggsave("/Users/Dagmar Heintze/Desktop/R-Directory/Coling/quality_scores.pdf", width = 5, height = 5.5, units = "in")
