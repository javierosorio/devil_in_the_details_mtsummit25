...
model.results <- model.results %>%
  mutate(mt.tool=case_when(
    lang=="EN" & trans.lang=="Native" & mt.tool=="Native" ~ "Native EN",
    lang=="ES" & trans.lang=="Native" & mt.tool=="Native" ~ "Native ES",
    lang=="AR" & trans.lang=="Native" & mt.tool=="Native" ~ "Native AR",
    TRUE ~ mt.tool)) %>%
...
ggplot(model.results,aes(x=estimate, y=term, color=mt.tool, group=mt.tool)) +
...
# Save plot 
ggsave(glue("graphs/reg/reg_ddm_std_v1.pdf"), 
...
# Plots
p.a <- ggplot(subset(model.results2, trans.lang=="(a) Native"),aes(x=estimate, y=term, color=mt.tool, group=mt.tool)) +
  geom_hline(yintercept = 0.4 , colour="black") +
  geom_hline(yintercept = 1.5 , colour="lightgray") +
  geom_hline(yintercept = 2.5 , colour="lightgray") +
  geom_hline(yintercept = 3.5 , colour="lightgray") +
...
pdf(glue("graphs/reg/reg_ddm_std_2cols_v1.pdf"), width = 3.8, height = 8.7) # Open a new pdf file
# Three rows grid
