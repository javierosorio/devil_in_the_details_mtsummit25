  ...
  for(y in unique.trans.lang){
    ...
    if (y=="Native"){
      ...
      for(z in unique.mt.tool){
        ...
        # Save plot 
        ggsave(glue("graphs/rmse/rmse_{x}_{y}_{z}.pdf"), 
...
model.results.pct <- model.results2 %>%
  ...
  mutate(fit.loss=1-(rmse/min.rmse)) %>%
  mutate(fit.loss = round(fit.loss, digits=5)) %>%
  arrange(lang, trans.lang, mt.tool, rmse) %>%
  mutate(trans.lang = case_when(
    trans.lang=="(a) Native" ~ "(a) NST", 
    TRUE ~ trans.lang  ))


# Plots 
p.a.pct <- ggplot(subset(model.results.pct, trans.lang=="(a) NST"),
  ...
  theme(legend.key.size = unit(0.3, "cm")) +
  labs(colour="") + 
  scale_color_manual(values=c("#003366", "#66cc99",
                              "#cc3333")) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
