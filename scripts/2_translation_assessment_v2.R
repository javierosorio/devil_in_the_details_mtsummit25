...
# Get the data
data.qual <- read.csv('https://raw.githubusercontent.com/javierosorio/translation_coling24/main/data/quality/master_quality_data_ID_Updated.csv',  header = TRUE)
...
data.qual.mean <- data.qual.mean %>%
  pivot_longer(cols=c("BLEU","SacreBLEU","METEOR","BERTScore"),
               names_to="metric",
               values_to="values")
