    ...
    for epoch in range(1, args.epochs_per_seed+1):
        ...
        if len(dirs) == 0:
            ...
            if args.task == "multilabel":
                ...
                result["prec"] = 0 if (result["tp"]+result["fp"]) == 0 else (float((result["tp"])/(result["tp"]+result["fp"])))
                result["rec"] = float((result["tp"])/(result["tp"]+result["fn"]))
                result["f1"] = 0 if (result["prec"]+result["rec"]) == 0 else (float(2*(result["prec"]*result["rec"])/(result["prec"]+result["rec"])))
